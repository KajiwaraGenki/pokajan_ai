...
with st.expander("🤖 AI解析について", expanded=False):
    st.markdown(
        """
        このツールは、見えていない相手の手札や山札を**複数のあり得る世界として再構成**し、
        それぞれの打牌からゲーム終了までを何度もシミュレーションします。

        打牌の評価は**順位ではなく、最終的に自分が持っているコインの期待値**を基準にしています。

        - **平均最終コイン**：その打牌を選んだ場合の平均的な最終所持コイン
        - **期待増減**：現在のコインから平均でどれだけ増減するか
        - **中央値**：極端な結果の影響を受けにくい代表値
        - **0点率**：シミュレーション上で0コインになった割合

        シミュレーション回数を増やすほど結果は安定しやすくなりますが、解析時間も長くなります。
        """
    )
...
st.caption(
    ...
    "解析結果はシミュレーションに基づく推定値であり、最善手を保証するものではありません。"
)
